...
v0.7.1 下拉选择与无输入动作识别修复：2026-09-18
v0.7.2 可编辑下拉项连续输入修复：2026-09-20

## v0.7.2 可编辑下拉项输入修复

0.7.1 把输入框隐藏在“新建位置…”选项后，并在输入值等于当前值时清除编辑状态。输入 `1` 后继续输入 `12` 时，输入框可能先被卸载，导致无法完成输入。现在只要原控件允许编辑，就始终显示输入框和下拉列表，两者共用同一个待提交值；空列表、选择已有值和原核心清空选中项均不阻止手动输入。

重新检查原始界面组件确认，保存位置和瞬间转移两项默认均可编辑。测试保留这两个真实属性，并将只读下拉框作为单独情形验证；前端测试也开始执行状态同步效果，覆盖连续数字、前导零、列表刷新和应用后的输入可用性。

## v0.7.1 位置控件修复

《黎明行者之血》的 `<combobox>` 现在作为文本选择控件贯穿 Bridge、后端和 Decky 面板，实时同步原控件的选项列表与可编辑能力。保存和传送沿用原生 `ExecuteTrainerCommand(ID, "")` 调用，原核心通过输入回调读取位置名称；重复选择同一位置也会执行。位置操作可能更新列表或清空选中项，因此完成条件为明确的已执行回执和新快照，不要求选中名称保持不变。

未知控件保留为禁用行，不再使整份菜单丢失；菜单和界面控件优先按 ID 对齐。菜单解析回退时仍可识别真实下拉控件，并检查输入框自身的 WPF 隐藏状态，使“瞬移到标记位置”保持纯动作。已使用 36 项版、37 项版的原始菜单和界面程序集验证，原生执行接口由测试替身记录，游戏内效果仍需 Steam Deck 实机验证。
