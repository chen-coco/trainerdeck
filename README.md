...
- 对风灵月影新版本（19 年+）修改器提供 Decky 控制面板，支持开关、数值输入、下拉选择和一次性动作。
...
0.7.2 修复了可编辑下拉项的输入问题：《黎明行者之血》的保存位置和瞬间转移同时显示下拉菜单与输入框，可直接输入数值或名称，也可选择已有值。连续输入、选择已有值及应用成功后，输入框都会保持显示；“瞬移到标记位置”仍只显示“应用”按钮。升级后请退出并重新启动游戏和修改器，以加载新版 Bridge。
...
当前版本对应 `release/TrainerDeck-0.7.2.zip`。
